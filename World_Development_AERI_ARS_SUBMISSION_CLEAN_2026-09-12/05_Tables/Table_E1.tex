...
\par\medskip\raggedright\footnotesize\textit{Notes:} Monthly AERI using the article-score definition in equation~\eqref{eq:aeri}. Only months with at least 100 retained articles are included. Means and sample standard deviations give equal weight to eligible months; they differ from article-weighted annual AERI. Peak denotes the maximum eligible monthly observation, with the earliest month reported in a tie. Countries are the same illustrative group as Figure~\ref{fig:annual}; PVAR sample selection is conducted separately.
\end{table}